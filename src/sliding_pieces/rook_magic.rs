...
// TODO make sure this matches the magic version
const fn rook_moves_iter(sq: Square, blockers: BitBoard) -> BitBoard {
...
pub const fn blocker_squares(sq: Square) -> BitBoard {
    const RANK_MASK: u64 = 0b0_111111_0;
...
const ROOK_TABLE_INDEX: [(usize, MagicValue); 64] = {
...
const ROOK_TABLE: [BitBoard; ROOK_TABLE_SIZE] = {
    ...
    while i < 64 {
        ...
        while let Some(blockers) = blockers.pop() {
            ...
            table[table_offset + index] = rook_moves_iter(sq, blockers);
        }
        i += 1;
    }
    table
...
pub fn magic_size(sq: Square, magic: MagicValue, max_size: usize) -> Option<usize> {
    ...
    for blockers in blocker_squares(sq).powerset() {
        let moves = rook_moves_magic(sq, blockers);
        let index = magic.to_index(blockers, 12);
        if index < max_size {
...
pub fn make_lut(
    ...
    index_bits: u8,
    magic: MagicValue,
) -> Option<(Vec<BitBoard>, usize)> {
    let mut lut = vec![BitBoard::EMPTY; 1 << index_bits];
    let mut max_index = 0;
    for blockers in blocker_squares.powerset() {
        let moves = rook_moves_iter(sq, blockers);
        let index = magic.to_index(blockers, index_bits);
        let entry = &mut lut[index];
        if *entry == BitBoard::EMPTY {
            *entry = moves;
...
pub mod tests {
    ...
    use crate::{
        bitboard::BitBoard,
        coord::Square,
        sliding_pieces::rook_magic::{
            blocker_squares, index_bits, rook_moves_iter, rook_moves_magic,
        },
    };
    ...
    fn rook_moves_iter_no_mask_is_const(sq: Square) -> bool {
        ...
        let bb = super::rook_moves_iter(sq, BitBoard::EMPTY);
        bb == bb_ref
    }
    ...
    fn rim_blockers_dont_matter(sq: Square, blockers: BitBoard) -> bool {
        BitBoard::RIM.contains(sq)
            || rook_moves_iter(sq, BitBoard::RIM.intersect(blockers))
                == rook_moves_iter(sq, BitBoard::EMPTY)
    }

    #[quickcheck]
    fn rook_moves_magic_matches_ref(sq: Square, blockers: BitBoard) -> bool {
        let blockers = blockers.unset(sq);
        rook_moves_iter(sq, blockers) == rook_moves_magic(sq, blockers)
    }
